 ...
 interface ChatContextType {
   ...
   branchFromMessage: (chatId: string, messageId: number) => Promise<string>;
 ...
 export const ChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
   ...
   const branchFromMessage = useCallback(async (chatId: string, messageId: number) => {
     if (!user) return '';
     const sourceChat = await db.chats.get(chatId);
     ...
     const sourceMessages = await db.messages.where('chatId').equals(chatId).sortBy('createdAt');
     const targetIndex = sourceMessages.findIndex(m => m.id === messageId);
     if (targetIndex < 0) return '';
     const prefixMessages = sourceMessages.slice(0, targetIndex + 1);
